class Board:
    ...
    def displayBoard(self):
        for i in range(0, 9, 3):
            print(
                f"{self._getCellColor(self.board[i])} | {self._getCellColor(self.board[i + 1])} | {self._getCellColor(self.board[i + 2])}"
            )
            if i < 6:
                print("--+---+--")

    def _getCellColor(self, cell):
        if cell == "O":
            return "\033[94mO\033[0m"  # Blue for player 1 (O)
        elif cell == "X":
            return "\033[91mX\033[0m"  # Red for player 2 (X)
        else:
            return " "

# Example of how to play:
board = Board("player2")
board.playMoveOnBoard(0, "player1")
board.playMoveOnBoard(4, "player2")
board.displayBoard()
